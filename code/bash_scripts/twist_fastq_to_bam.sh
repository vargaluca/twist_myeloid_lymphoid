...
covered='/disk/work/diagnostics/DLBC/lv1/DLBCL_genclass_K21/bed_files/S3405393/DLBCL_final_comprehensive_panel_1_Covered_corrected.bed'
	...
	for bamfile in *sorted.bam
		...
		samtools idxstats $bamfile > $path/bam_qc/$run/$outdir/$(echo $bamfile | cut -f 1 -d "_").idxstats.txt
                samtools flagstat $bamfile > $path/bam_qc/$run/$outdir/$(echo $bamfile | cut -f 1 -d "_").flagstat.txt
		echo $(echo $bamfile | cut -f 1 -d "_") bamqc finished >> $path/log_files/$run.log.txt
	done

cd $path/bam_qc/$run

multiqc -n $path/bam_qc/$run/reports/$run.dlbcl-multiqc --file-list file_paths.txt
...
module load gatk
	...
	for bamfile in *.bam
		...
		gatk BaseRecalibrator -I $bamfile -R $reference \
        	-O $path/recal_aligned_reads/$run/$(echo $bamfile | sed 's/deduplexed.bam/recal_data.table/') \
        	--known-sites $known1 \
        	--known-sites $known2 \
        	--known-sites $known3
...
echo RECALIBRATION TABLES FINISHED ON $(date) >> $path/log_files/$run.log.txt
	...
	for bamfile in *.bam
		...
		gatk ApplyBQSR -R $reference -I $bamfile \
        	--bqsr-recal-file $path/recal_aligned_reads/$run/$(echo $bamfile | sed 's/deduplexed.bam/recal_data.table/') \
        	-O $path/recal_aligned_reads/$run/$(echo $bamfile | sed 's/deduplexed.bam/recalibrated.bam/')
